...
def admin_delete_user_account(user_id: str, actor: str = "admin") -> Dict[str, Any]:
    """
    Permanently remove a self-registered app account.

    Deliberately scoped to app_users ONLY, looked up by exact user_id — unlike
    _find_account (used for password-reset/unlock) this never falls through
    to admin_users. That table holds the Technical Assistant's own sign-in,
    and letting a fuzzy mobile/email/username match land there would risk an
    admin deleting the account they are currently using to administer the app.
    """
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT id, name, mobile_number FROM app_users WHERE user_id = ?", (user_id,))
        row = cursor.fetchone()
        if not row:
            return {"success": False, "message": "No registered account found with that ID."}

        display_name = _row_value(row, "name", "") or user_id
        mobile = _row_value(row, "mobile_number", "") or ""

        cursor.execute("DELETE FROM app_users WHERE id = ?", (row["id"],))
        # Same identifier used to key the daily search-quota table — no
        # reason to keep those rows once the account itself is gone.
        cursor.execute("DELETE FROM user_search_quota WHERE user_identifier = ?", (user_id,))
        conn.commit()

        audit("admin_account_deleted", mobile or user_id, "success",
              f"deleted registered account for {display_name} ({user_id})", actor=actor)

        return {"success": True, "message": f"Account for {display_name} deleted."}
    finally:
        conn.close()
