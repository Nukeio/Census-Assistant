...
#
# "gemini-flash-latest" leads the list: on this deployment's API key, the
# pinned versions below it consistently 404 ("model not available") while the
# alias resolves fine, so trying the pinned ones first only burns two
# guaranteed-failed calls before ever reaching the model that works. Keeping
# them as fallbacks costs nothing and covers a key/region where the alias
# itself is briefly unavailable.
...
def call_gemini_api(
    ...
    # Per-attempt reasons, so that if every model fails the final message can
    # say *why* each one failed instead of the generic "exhausted" wording
    # that used to silently overwrite whatever specific error (timeout, empty
    # response, odd HTTP status) happened on the last model tried.
    ...
    for attempt, candidate_model in enumerate(models_to_try):
        ...
        if resp is None:
            failure_notes.append(f"{candidate_model}: network error after retry")
        ...
        if resp.status_code == 200:
            ...
            failure_notes.append(f"{candidate_model}: HTTP 200 but no text in response")
            continue
        ...
        if resp.status_code == 503:
            # Google's own message for this status says demand spikes are
            # "usually temporary" — worth a couple of short retries on the
            # same model before writing it off as unavailable and moving on.
            for backoff in (2, 4):
                time.sleep(backoff)
                try:
                    resp = requests.post(url, json=body, timeout=(10, 45), proxies=REQUEST_PROXIES)
                except Exception as exc:
                    _record_provider_error("network", f"{candidate_model} (503 retry): {exc}")
                    break
                if resp.status_code == 200:
                    answer = _extract(resp.json())
                    if answer:
                        logger.info(f"Gemini answered on {candidate_model} after a 503 retry.")
                        return answer, was_web_searched, candidate_model
                    break
                if resp.status_code != 503:
                    break
            _record_provider_error("overloaded", f"{candidate_model}: HTTP 503 (overloaded) even after retries")
            failure_notes.append(f"{candidate_model}: HTTP 503 (overloaded), gave up after retries")
            continue

        if resp.status_code == 404:
            logger.info(f"Model {candidate_model} not available; trying next.")
            failure_notes.append(f"{candidate_model}: HTTP 404 (model not available)")
            continue

        _record_provider_error("http", f"{candidate_model}: HTTP {resp.status_code} {resp.text[:200]}")
        failure_notes.append(f"{candidate_model}: HTTP {resp.status_code} {resp.text[:150]}")

    detail = "; ".join(failure_notes) if failure_notes else "no models were attempted"
    _record_provider_error("exhausted", f"No Gemini model produced an answer. Attempts — {detail}")
    return None, was_web_searched, None
