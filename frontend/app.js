...
const state = {
  ...
  activeChatAbortController: null, // lets the chat Stop button cancel the in-flight /api/chat fetch
...
async function handleSendChat(e) {
  ...
  // Append thinking skeleton, with a Stop control the user can actually use.
  // A previous request had no way to cancel and no indication that leaving
  // the screen mid-answer was a bad idea, so a slow Gemini response looked
  ...
  try {
    ...
    if (err.name === "AbortError") {
      appendChatMessage("ai", "Stopped — you cancelled this request before it finished.", []);
    } else {
...
/**
 * Cancel the in-flight /api/chat request from the Stop button shown next to
 * the thinking indicator. Note this only stops the BROWSER from waiting on
 * the response — the Gemini call already sent to the backend keeps running
 * server-side until it finishes or times out, since the API is a single
 * request/response call rather than a token stream that can be interrupted
 * mid-generation. What the user gets back immediately is control of the
 * screen, not a guarantee the backend stopped working.
 */
...
function appendChatMessage(sender, text, citations = [], intent = "", webSearched = false,
  ...
  if (sender === "user") {
    // Short right-aligned bubble — jumping to the bottom is correct here,
    // it's what the user just typed, and a "thinking" skeleton is about to
    // appear right below it.
    container.scrollTop = container.scrollHeight;
  } else {
    // A long assistant answer used to land on scrollHeight too, which shows
    // the TAIL of the message (footer / sources / feedback buttons) instead
    // of where the answer actually starts. Scroll so the top of the new
    // message is what comes into view.
...
function appendChatSkeleton() {
  ...
  skel.id = id;
  skel.className = "flex flex-col gap-2 max-w-[90%] md:max-w-[80%] animate-fade-in";
  skel.innerHTML = `
    ...
    <div class="flex items-center justify-between gap-2 pl-11 pr-1">
      <p class="text-[11px] text-[#8a6100] font-medium leading-snug">
        The AI is thinking — please wait, don't close or refresh this screen.
      </p>
      <button type="button" onclick="stopChatGeneration()"
        class="shrink-0 flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold border border-outline-variant text-on-surface-variant hover:bg-surface-container active:scale-95 transition-transform">
...
async function loadAdminAccounts() {
  ...
  try {
    ...
    const data = await res.json();
    if (!data.success) {
      tbody.innerHTML = `<tr><td colspan="7" class="p-4 text-center text-error">Could not load accounts.</td></tr>`;
      return;
    }
    if (!data.accounts.length) {
      tbody.innerHTML = `<tr><td colspan="7" class="p-4 text-center text-on-surface-variant">No password accounts registered yet.</td></tr>`;
      return;
    }
    ...
    data.accounts.forEach(a => {
      ...
      tr.className = "hover:bg-surface-container-low transition-colors";
      tr.innerHTML = `
        <td class="p-3 font-bold text-on-surface cursor-pointer" onclick="document.getElementById('support-identifier').value = '${escapeAttr(a.mobile_number || a.user_id)}'">${escapeHtml(a.name)}</td>
        <td class="p-3 text-on-surface-variant cursor-pointer" onclick="document.getElementById('support-identifier').value = '${escapeAttr(a.mobile_number || a.user_id)}'">${escapeHtml(a.mobile_number || "—")}</td>
        ...
        <td class="p-3">
          <button onclick="handleAdminDeleteAccount('${escapeAttr(a.user_id)}', '${escapeAttr(a.name)}')"
            class="p-1.5 rounded-full text-error hover:bg-error/10 transition-colors" title="Delete this account">
            <span class="material-symbols-outlined text-base">delete</span>
          </button>
        </td>`;
      tbody.appendChild(tr);
...
async function handleAdminDeleteAccount(userId, name) {
  if (!confirm(`Permanently delete the registered account for ${name}?\n\nThis removes their sign-in — it does NOT delete any census/HLB records or attendance history tied to them. This cannot be undone.`)) {
    return;
  }
  try {
    ...
    const data = await res.json();
    showToast(data.message || (data.success ? "Account deleted." : "Could not delete account."));
    if (data.success) loadAdminAccounts();
  } catch (err) {
